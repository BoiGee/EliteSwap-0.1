...
-- Repricing: internal cost-basis reporting used 2.083 credits/sec
-- (= 5000 credits / 2400s, Decart's actual per-second cost). Bump the
-- reporting rate to 3.2 credits/sec to build in margin.
--
-- Scope check: credits_per_second is NOT consumed anywhere in the actual
-- billing path (mint_studio_credentials / heartbeat_studio_session /
-- pause_studio_session / reap_orphaned_studio_sessions all deduct real
-- wall-clock milliseconds from api_keys.remaining_ms, independent of this
-- constant). Its only two readers are admin_time_ledger() — which powers
-- the admin-only Time Ledger dashboard — and the one-time reconciliation
-- backfill's audit-log math. Neither is user-facing today, so this change
-- is already invisible to end users: nobody outside admin sees "credits" at
-- all, only remaining minutes. If the intent was instead to make purchased
-- studio time actually run out faster than real elapsed seconds (a change
-- to the ms-deduction math itself, not just this reporting multiplier),
-- that's a separate, materially bigger change this migration does NOT make.
ALTER TABLE public.studio_pricing_config
  ALTER COLUMN credits_per_second SET DEFAULT 3.2;

UPDATE public.studio_pricing_config
   SET credits_per_second = 3.2,
       updated_at = now()
 WHERE id = true;
