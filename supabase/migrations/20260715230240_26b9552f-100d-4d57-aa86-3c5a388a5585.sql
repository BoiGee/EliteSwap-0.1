...
BEGIN
  ...
  -- If a prior session is still recorded on this key, decide: fresh (block)
  -- vs stale/orphaned (reap inline so this user's fresh tab isn't stuck).
  IF v_row.active_session_id IS NOT NULL THEN
    ...
    IF FOUND AND v_prior.last_heartbeat_at > now() - interval '15 seconds' THEN
      -- Live tab in another window — legitimate guard, keep blocking.
    ...
    IF FOUND THEN
      -- Stale prior row: bill it using the same formula as mint_studio_credentials.
      ...
      UPDATE public.studio_sessions AS s
       ...
       WHERE s.session_id = v_prior.prior_session_id AND s.ended_at IS NULL;
      ...
      UPDATE public.api_keys
       ...
       WHERE id = v_row.id;
    ELSE
      -- Orphaned lock: no matching studio_sessions row at all. Just clear it.
      UPDATE public.api_keys
    ...
    -- Re-read the key after reap so subsequent logic uses fresh state.
    SELECT * INTO v_row FROM public.api_keys WHERE id = v_row.id FOR UPDATE;

    -- Re-check exhaustion in case the reap billed the key to zero.
