...
-- =========================================================
-- Round 4: pricing config + tightened metering
-- =========================================================

-- 1) Pricing config (single-row, admin-tunable)
CREATE TABLE IF NOT EXISTS public.studio_pricing_config (
  id boolean PRIMARY KEY DEFAULT true CHECK (id = true),
  credits_per_second numeric NOT NULL DEFAULT (5000.0 / 2400.0),
  handshake_floor_ms bigint NOT NULL DEFAULT 8000,
  heartbeat_grace_ms bigint NOT NULL DEFAULT 1500,
  reap_stale_ms bigint NOT NULL DEFAULT 8000,
  updated_at timestamptz NOT NULL DEFAULT now()
);

GRANT SELECT ON public.studio_pricing_config TO authenticated;
GRANT ALL ON public.studio_pricing_config TO service_role;

ALTER TABLE public.studio_pricing_config ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "Anyone authenticated can read pricing config" ON public.studio_pricing_config;
CREATE POLICY "Anyone authenticated can read pricing config"
  ON public.studio_pricing_config FOR SELECT TO authenticated USING (true);
...
INSERT INTO public.studio_pricing_config (id)
VALUES (true)
ON CONFLICT (id) DO NOTHING;

-- 2) Reporting helper: how many Decart credits a duration in ms represents
CREATE OR REPLACE FUNCTION public.studio_credits_for_ms(p_ms bigint)
RETURNS numeric
LANGUAGE sql
STABLE
SECURITY DEFINER
...
AS $$
  SELECT COALESCE(p_ms, 0)::numeric / 1000.0
         * COALESCE((SELECT credits_per_second FROM public.studio_pricing_config LIMIT 1), 5000.0/2400.0)
$$;

-- 3) mint_studio_credentials — bump min_bill_ms floor to config value
...
BEGIN
  ...
  IF v_row.active_session_id IS NOT NULL THEN
    SELECT s.session_id, s.started_at, s.last_heartbeat_at,
    ...
    IF FOUND THEN
      ...
      UPDATE public.studio_sessions
       ...
       WHERE session_id = v_prior.session_id AND ended_at IS NULL;
      ...
      SELECT * INTO v_row FROM public.api_keys WHERE id = v_row.id FOR UPDATE;
      IF v_row.remaining_ms IS NOT NULL AND v_row.remaining_ms <= 0 THEN
        UPDATE public.api_keys SET is_active = false WHERE id = v_row.id;
        RETURN QUERY SELECT false, 'expired_or_inactive'::text, NULL::text, v_row.remaining_ms, v_row.label, v_is_trial, NULL::text, NULL::timestamptz;
        RETURN;
      END IF;
    ELSE
...
-- 4) start_studio_session — same min_bill_ms from config
...
BEGIN
  ...
  IF v_row.active_session_id IS NOT NULL THEN
    SELECT last_heartbeat_at INTO v_prior
      FROM public.studio_sessions
     WHERE session_id = v_row.active_session_id AND ended_at IS NULL
     LIMIT 1;
...
-- 5) heartbeat_studio_session — auto-close on wall-clock overrun
...
BEGIN
  ...
  UPDATE public.studio_sessions
   ...
   RETURNING started_at, remaining_ms_at_start, COALESCE(min_bill_ms, v_min_bill)
...
-- 6) reap_orphaned_studio_sessions — 8s stale window, advisory-lock guard
...
-- 7) Reschedule cron from 10s to 5s
DO $$
BEGIN
