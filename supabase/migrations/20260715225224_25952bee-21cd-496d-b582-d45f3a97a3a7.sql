...
BEGIN
  ...
  IF v_row.active_session_id IS NOT NULL THEN
    SELECT s.last_heartbeat_at AS last_heartbeat_at INTO v_prior
      FROM public.studio_sessions s
     WHERE s.session_id = v_row.active_session_id AND s.ended_at IS NULL
     LIMIT 1;
...
BEGIN
  ...
  IF v_row.active_session_id IS NOT NULL THEN
    SELECT s.session_id AS prior_session_id, s.started_at AS started_at, s.last_heartbeat_at AS last_heartbeat_at,
