...
-- Round 3 tightening: kill remaining studio-metering leaks.
-- A) serialize per-key sessions (multi-tab burn)
-- B) debit on pause even when no expires_at
-- C) remove 60s resume path in start_studio_session
-- D) minimum bill floor per session

BEGIN;

ALTER TABLE public.studio_sessions
  ADD COLUMN IF NOT EXISTS min_bill_ms bigint NOT NULL DEFAULT 3000;

-- ============================================================
-- mint_studio_credentials: serialize + takeover-with-debit
-- ============================================================
...
BEGIN
  ...
  -- Serialize: if there's an open session on this key, decide takeover vs block.
  IF v_row.active_session_id IS NOT NULL THEN
    SELECT s.session_id, s.started_at, s.last_heartbeat_at,
           ...
           COALESCE(s.min_bill_ms, 3000) AS min_bill
      INTO v_prior
    ...
    IF FOUND THEN
      -- Live if heartbeat within 15s. Block.
      ...
      -- Prior is stale: close it and debit before taking over.
      ...
      -- Re-read to pick up the debited balance.
      ...
      END IF;
    ELSE
      -- Orphan lock (no matching session row): clear it.
      PERFORM set_config('app.bypass_key_guard', 'on', true);
      UPDATE public.api_keys
...
-- ============================================================
-- pause_studio_session: debit even when no expires_at, apply min-bill floor
-- ============================================================
...
BEGIN
  ...
  SELECT started_at, remaining_ms_at_start, COALESCE(min_bill_ms, 3000) AS min_bill
    INTO v_session
  ...
  ELSIF v_row.expires_at IS NOT NULL THEN
    ...
    -- Also apply bill floor against current remaining if we have one.
...
-- ============================================================
-- start_studio_session: remove 60s resume path
-- ============================================================
...
BEGIN
  ...
  -- Serialize: block if a session is live (heartbeat within 15s). No adoption/resume.
  IF v_row.active_session_id IS NOT NULL THEN
    ...
    IF FOUND AND v_prior.last_heartbeat_at > now() - interval '15 seconds' THEN
      ...
      RETURN;
    END IF;
    -- Stale lock: let the reaper / next mint sort it. Don't silently adopt.
    RETURN QUERY SELECT false, 'already_active_elsewhere'::text, NULL::text, NULL::timestamptz, v_row.remaining_ms, v_row.label, v_is_trial;
    RETURN;
  END IF;
